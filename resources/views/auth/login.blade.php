    ...
    <form method="POST" action="{{ route('login.submit') }}" class="space-y-4">
        ...
        @if($enableGoogle || $enableGithub)
            ...
            <div class="grid grid-cols-{{ ($enableGoogle && $enableGithub) ? '2' : '1' }} gap-3">
                ...
                @if($enableGithub)
                    <a href="{{ route('auth.github', ['action' => 'login']) }}" class="flex items-center justify-center gap-2 px-4 py-2.5 border border-slate-800 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-semibold transition-colors shadow-2xs">
