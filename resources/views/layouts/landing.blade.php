        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
            ...
            <div class="flex items-center gap-2 sm:gap-2.5">
                ...
                <button @click="toggleTheme()" 
                        type="button" 
                        class="p-2 text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors cursor-pointer border border-slate-200/80 dark:border-slate-800" 
                        :title="darkMode ? 'Beralih ke Mode Terang (Light)' : 'Beralih ke Mode Gelap (Dark)'"
                        aria-label="Toggle Light/Dark Theme">
                    <template x-if="darkMode">
                        <span class="flex items-center gap-1 text-xs font-semibold font-mono text-amber-400">
                            <i data-lucide="sun" class="w-4 h-4"></i>
                            <span class="hidden lg:inline text-[11px]">Light</span>
                        </span>
                    </template>
                    <template x-if="!darkMode">
                        <span class="flex items-center gap-1 text-xs font-semibold font-mono text-slate-600">
                            <i data-lucide="moon" class="w-4 h-4 text-slate-600"></i>
                            <span class="hidden lg:inline text-[11px]">Dark</span>
                        </span>
                    </template>
                </button>
