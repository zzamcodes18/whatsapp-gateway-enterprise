    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b border-slate-200/80 dark:border-slate-800 pb-4">
        <div>
            <div class="flex items-center gap-2">
                ...
                <span class="font-mono text-[11px] font-semibold text-slate-500 dark:text-slate-400">SYSTEM TELEMETRY</span>
            </div>
            <h1 class="font-extrabold text-xl sm:text-2xl mt-1 text-navy">Admin Control Center</h1>
            <p class="text-xs text-slate-500 dark:text-slate-400 font-medium">Monitoring seluruh pengguna, kuota harian, semua perangkat, dan server bot OTP.</p>
        </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="app-card p-4 sm:p-5 space-y-2">
            ...
            <div class="flex items-baseline gap-1.5">
                ...
                <span class="text-xs text-slate-400 dark:text-slate-500 font-semibold">({{ $activeUsers }} Aktif)</span>
            </div>
        ...
        <div class="app-card p-4 sm:p-5 space-y-2">
            ...
            <div class="flex items-baseline gap-1.5">
                ...
                <span class="text-xs text-slate-400 dark:text-slate-500 font-semibold">/ {{ $totalDevices }} Total</span>
            </div>
        ...
        <div class="app-card p-4 sm:p-5 space-y-2">
            ...
            <div class="flex items-baseline gap-1.5">
                ...
                <span class="text-xs text-slate-400 dark:text-slate-500 font-semibold">Pesan</span>
            </div>
            <div class="text-[11px] font-medium text-slate-500 dark:text-slate-400">
                Total se-sistem: <strong class="text-slate-800 dark:text-slate-200">{{ $totalMessages }}</strong>
            </div>
        </div>
        ...
        <div class="app-card p-4 sm:p-5 space-y-2">
            ...
            <div class="flex items-baseline gap-1.5">
                @if($botDevice && $botDevice->isConnected())
                    <span class="font-bold text-sm text-emerald-700 dark:text-emerald-400">ONLINE TERHUBUNG</span>
                @elseif($botDevice)
                    <span class="font-bold text-sm text-amber-600 dark:text-amber-400">OFFLINE</span>
                @else
                    <span class="font-bold text-sm text-rose-600 dark:text-rose-400">BELUM DIATUR</span>
                @endif
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
                        ...
                        @foreach($recentUsers as $usr)
                            <tr>
                                <td class="p-3.5">
                                    ...
                                    <div class="font-mono text-[11px] text-slate-500 dark:text-slate-400">{{ $usr->email }}</div>
                                </td>
                                <td class="p-3.5">
                                ...
                                <td class="p-3.5 font-mono font-bold text-slate-800 dark:text-slate-200">{{ $usr->device_limit > 0 ? $usr->device_limit . ' Unit' : 'Unlimited' }}</td>
                                <td class="p-3.5 font-mono font-bold text-slate-800 dark:text-slate-200">{{ $usr->daily_message_limit ? $usr->daily_message_limit . ' msg' : 'Unlimited' }}</td>
                                <td class="p-3.5">
        ...
        <div class="lg:col-span-5 space-y-3.5">
            <div class="flex items-center justify-between">
                ...
                <span class="font-mono text-[10px] font-semibold text-slate-400 dark:text-slate-500">REALTIME AUDIT</span>
            </div>

            <div class="app-card p-4 bg-white dark:bg-[#111A2E] space-y-2.5">
                @forelse($recentLogs as $log)
                    <div class="p-3 bg-slate-50/80 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800 rounded-xl text-[11px] space-y-1">
                        <div class="flex items-center justify-between font-semibold">
                            <span class="font-mono text-[9px] bg-white dark:bg-[#0D1526] px-2 py-0.5 border border-slate-200 dark:border-slate-700 rounded text-slate-700 dark:text-slate-300">{{ $log->event }}</span>
                            <span class="text-[10px] text-slate-400 dark:text-slate-500 font-mono">{{ $log->created_at->diffForHumans() }}</span>
                        </div>
                        <p class="text-slate-800 dark:text-slate-200 font-medium text-[11px]">{{ $log->description }}</p>
                        @if($log->user)
                            <div class="text-[10px] text-slate-500 dark:text-slate-400 font-mono">User: {{ $log->user->email }}</div>
                        @endif
                    </div>
                @empty
                    <p class="text-xs text-slate-400 dark:text-slate-500 font-medium py-3 text-center">Belum ada log tercatat.</p>
                @endforelse
            </div>
        </div>
